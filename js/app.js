...
const MainApp = {
    ...
    state: {
        ...
        moduloActual: '', // M├│dulo principal (ej: 'inventario', 'tareas')
    },
    ...
    checkPermissions() {
        ...
        // Ocultar m├│dulo de tareas en el portal principal a roles no autorizados (ahora incluimos 'usuario')
        ...
        writeButtons.forEach(btn => {
            if (isVisualizer || isSupervisor) {
                btn.style.display = 'none'; // Ocultar para evitar tentaci├│n
            } else {
                btn.style.display = '';
            }
        });
    ...
    /**
     * Inicializa los eventos de clic del sidebar para el men├║ de inventario.
     * El men├║ de tareas usa 'onclick' directamente en el HTML para mayor claridad.
     */
    inicializarNavegacion() {
        // Navegaci├│n de Inventario
    ...
    setMode(mode) {
        ...
        // Garantizar que el perfil de usuario siga visible siempre que haya sesi├│n
        const session = api.getSession();
        ...
        if (mode === 'inventario') {
            ...
            document.getElementById('page-subtitle').textContent = 'Gesti├│n del almac├®n';
    ...
    /**
     * Especializado para cambiar entre las sub-vistas del m├│dulo de Tareas.
     * @param {string} viewName - Nombre de la subview de tareas
     */
    switchTareasView(viewName) {
        // En nuestro nuevo dise├▒o, las sub-vistas son vistas de primer nivel en el sidebar
        ...
        // Actualizar t├¡tulos seg├║n la vista
        const titles = {
            'dashboard': ['Dashboard de Cumplimiento', 'Indicadores Clave de Rendimiento'],
            'recurrentes': ['Mantenimiento Mensual', 'Programaci├│n de actividades fijas por mes'],
            ...
            'usuarios': ['Responsables de Mantenimiento', 'Gesti├│n del personal y equipos asignados'],
            'bitacora': ['Bit├ícora de Evidencias', 'Registro fotogr├ífico de actividades realizadas']
        };
    ...
    async cargarTodosLosDatos() {
        if (GAS_URL.includes('TU_URL_WEB_APP')) {
            ...
            utils.mostrarToast('ÔÜá´©Å Configura la URL de tu Web App en api.js', 'warning');
            this.usarDatosDemo();
            return;
        }
        ...
        try {
            ...
            ui.setConexionStatus('error');
            utils.mostrarToast('Error de conexi├│n: ' + err.message, 'danger');
        } finally {
            utils.ocultarLoader();
        }
    },
    ...
    /**
     * Sincronizaci├│n r├ípida: Solo carga productos y movimientos.
     * Ideal para operaciones de almac├®n que no requieren datos de tareas.
     */
    ...
    // Renderiza la matriz preventiva llamando a la funci├│n en tareasUI.js
    renderizarPreventivoMatriz() {
        ui.renderizarMatrizPreventivo(this.state.planPreventivo, this.state.usuariosPreventivo);
    },

    // Abre el modal para registrar un mantenimiento en una celda usuario├ùmes├ùsemana
    abrirModalPrevReg(usuarioId, nombreUsuario, mes, semana) {
        ...
        document.getElementById('prev-reg-periodo').textContent = `${MESES[Number(mes)-1]} ÔÇô Semana ${semana}`;
    ...
    async guardarRegistroPreventivo(e) {
        ...
        try {
            ...
            const res = await api.post({ 
                ...
                registro: {
                    id: existingId || ('PREV-' + Date.now()), // Si existe ID, se mantiene para actualizaci├│n
                    Usuario: nombre, 
                    UsuarioId: usuarioId, 
                    Mes: mes,
                    Semana: semana,
            ...
            utils.mostrarToast('Mantenimiento guardado Ô£ô', 'success');
        } catch (err) {
    ...
    async eliminarRegistroPreventivo() {
        ...
        if (!confirm('┬┐Est├ís seguro de eliminar este registro de mantenimiento?')) return;
        ...
        try {
            ...
            await this.cargarTodosLosDatos();
            this.renderizarPreventivoMatriz();
            utils.mostrarToast('Registro eliminado con ├®xito', 'success');
        } catch (err) {
    ...
    async eliminarUsuarioPreventivo(id, nombre) {
        if (!id) return;
        if (!confirm(`┬┐Est├ís seguro de eliminar a "${nombre}" del directorio preventivo?`)) return;
    ...
    async registrarUsuarioPreventivo(e, nombreProp, areaProp) {
        ...
        if (!nombre) {
            utils.mostrarToast('Nombre del t├®cnico es obligatorio', 'warning');
            return;
        }
        ...
        try {
            ...
            await this.cargarTodosLosDatos();
            this.renderizarPreventivoMatriz();
            utils.mostrarToast('Usuario agregado con ├®xito Ô£ô', 'success');
        } catch (err) {
    ...
    agregarUsuarioMatriz() {
        // Alias para el bot├│n r├ípido de la matriz
        ...
        if (!nombre) {
            utils.mostrarToast('Escribe el nombre del t├®cnico en el buscador para agregarlo', 'warning');
            return;
        }
        this.registrarUsuarioPreventivo(null, nombre, area || 'Producci├│n');
    },

    usarDatosDemo() {
        this.state.productos = [
            { ID: 'PRD-001', Nombre: 'Laptop Dell XPS', Categoria: 'Electr├│nica', Descripcion: 'Demo', Cantidad: 15, Unidad: 'Unidades' },
            { ID: 'PRD-002', Nombre: 'Monitor 27"', Categoria: 'Electr├│nica', Descripcion: 'Demo', Cantidad: 4, Unidad: 'Unidades' }
        ];
        this.state.entradas = [];
        this.state.salidas = [];
        this.state.entregas = [];
    ...
    inicializarEventos() {
        ...
        // Selecci├│n de filas (Resaltado Premium solicitado por el usuario)
            ...
            if (this.state.moduloActual === 'tareas') {
                if (this.state.tabTareasActual === 'bitacora') {
                    ...
                    const dataExport = this.state.tareasRecurrentes.map(t => {
                        // Normalizaci├│n robusta (igual que en ui.js)
        ...
        // Quick-add user form en la secci├│n Preventivo
        ...
        // Listeners Pesta├▒a Filtros Tareas
